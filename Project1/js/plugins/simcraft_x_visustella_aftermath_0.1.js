...
/*:
 * @target MZ
 * @plugindesc (v0.09) Ajoute une scène après un combat victorieux pour distribuer des points de stats.
...
this.setHandler("cancel", this.onCancel.bind(this));
         ...
         */
        makeCommandList() {
            this.addCommand("Valider", "ok", () => !this._confirmDelay);
            this.addCommand("Annuler", "cancel", () => !this._confirmDelay);
        }

        /**
         * Gestion de la validation après délai.
         */
        onOk() {
            if (!this._confirmDelay) {
                SoundManager.playOk();
                this.callOkHandler();
            }
        }
        ...
        /**
         * Gestion de l'annulation après délai.
         */
        onCancel() {
            if (!this._confirmDelay) {
        ...
        /**
         * Effet visuel lors de l'affichage de la fenêtre.
         */
        fadeInEffect() {
            const duration = 20;
            this.updateFadeIn(duration);
        }

        updateFadeIn(duration) {
            const step = 255 / duration;
            let count = 0;
            const fadeInterval = setInterval(() => {
                this.opacity += step;
                this.contentsOpacity += step;
                count++;
                if (count >= duration) clearInterval(fadeInterval);
            }, 16);
        }
        showConfirm() {
            this.show();
...
applyStats() {
            for (const [key, tempStat] of Object.entries(this._tempStats)) {
                ...
                if (valueDiff > 0) originalStat.addStatLevel(valueDiff);
            }
